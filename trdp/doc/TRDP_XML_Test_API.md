...
## 1. XML Document & Configuration Helpers
...
1. `tlp_getInterval()` → prepares `select()` timeout
2. `tlp_processReceive()` → handles inbound packets
3. `tlp_processSend()` → handles periodic transmit engine
